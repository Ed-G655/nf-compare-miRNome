...
files = list.files(pattern="*.tsout")
targetscan.df = do.call(rbind, lapply(files, fread))
rm(files)

# transform data to df
targetscan.df <- as.data.frame(unclass(targetscan.df))



# import targetscan files 
files = list.files(pattern="*.mirmapout")
mirmap.df = do.call(rbind, lapply(files, fread))
rm(files)

# transform data to df
mirmap.df <- as.data.frame(unclass(mirmap.df))
