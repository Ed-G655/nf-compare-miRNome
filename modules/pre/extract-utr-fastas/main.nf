...
process EXTRACT_UTR_FASTA {
	tag "$BED, $FASTA.baseName"

	publishDir "${intermediates_dir}/extract-fasta/",mode:"symlink"
	...
	echo "[DEBUG] Extracting ${FASTA} into ${FASTA.baseName}.utr"
	bedtools getfasta -fi ${FASTA} -bed ${BED} -name -s -fo ${FASTA.baseName}.tmp
	echo "[DEBUG] Change DNA to RNA"
	less -s ${FASTA.baseName}.tmp | perl -pe 'tr/T/U/ unless(/>/)' > ${FASTA.baseName}.utr

	"""
	stub:
	"""
	      touch ${FASTA.baseName}.utr
	"""
}
